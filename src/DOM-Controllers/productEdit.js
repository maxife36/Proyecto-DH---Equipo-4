...
function updateWarnings() {
    ...
    if ( addImg.files.length == 0 && !imageSelector[0]) {
...
function changeImage(event) {
    ...
    if (selectedImageTitle.startsWith('blob:')) {
...
editForm.addEventListener("submit", async (event) => {
    ...
    const productId = editForm.querySelector("#productId")?.value
    const productName = editForm.querySelector("#productName")?.value
    const shortDescription = editForm.querySelector("#shortDescription")?.value
    const productBrand = editForm.querySelector("#productBrand")?.value
    const productPrice = editForm.querySelector("#productPrice")?.value
    const stock = editForm.querySelector("#stock")?.value
    const discount = editForm.querySelector("#discount")?.value
    const longDescription = editForm.querySelector("#longDescription")?.value
    ...
    const categoryInput = editForm.querySelector("#productCategory")
    ...
    const features = []
    const allFeatureArticles = editForm.querySelectorAll(".item-feature")
